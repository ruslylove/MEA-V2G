#!/usr/bin/env python3
"""
Generate a LaTeX/PDF test report for MEA OCPP 1.6 Section 9.
Reads  tex/vsecc_section9_results.json  (produced by test_mea_section9.py)
Writes tex/vsecc_section9_report.tex
Compiles tex/vsecc_section9_report.pdf  (requires pdflatex)

Usage (from project root):
  python3 tests/vsecc/generate_section9_report.py
"""
import json
import os
import subprocess
...
RESULTS_JSON = os.path.join(_ROOT, "tex", "vsecc_section9_results.json")
OUTPUT_TEX   = os.path.join(_ROOT, "tex", "vsecc_section9_report.tex")
PDF_NAME     = "vsecc_section9_report.pdf"
...
# Subsection dividers
SUBSECTION_HEADERS = {
    "9.1":   r"9.1--9.3 Configuration (Standard Keys via ChangeConfiguration)",
    "9.4.0": r"9.4 Power Demand (V2G/BPT) --- MEA-Specific Keys",
}
...
def build_tex(data: dict) -> str:
    ...
    meter_wait     = data.get("meter_wait_sec", "?")
...
\textbf{MeterValues Timeout} & """ + str(meter_wait) + r""" s per power-demand item \\
...
def main():
    if not os.path.exists(RESULTS_JSON):
        print(f"ERROR: {RESULTS_JSON} not found.  Run test_mea_section9.py first.")
        return
