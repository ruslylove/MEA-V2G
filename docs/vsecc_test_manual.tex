...
\begin{tabular}{C{1cm} L{4cm} C{2cm} L{7cm}}
\toprule
\textbf{Sec.} & \textbf{Title} & \textbf{EV needed} & \textbf{Notes} \\
\midrule
2  & Auto Charge           & No        & Verifies OCPP boot + auto-auth \\
3  & Normal Operation      & Yes (3.3+) & Local RFID + RemoteStart flows \\
4  & Reset Check           & Yes (4.4+) & Hard \& Soft Reset; vSECC reboots \\
5  & Reservation           & Yes (5.11+) & Reserve$\to$Cancel, Expiry, Charge \\
6  & Charging Profile      & Yes (6.2+) & SetChargingProfile during session \\
7  & Abnormal Operation    & Yes (7.2+) & E-stop, door, power loss, local list \\
8  & Dual Connector        & N/A        & All SKIP — single connector device \\
9  & V2G / BPT             & No         & ChangeConfiguration + metervalues \\
10 & CSMS Commands         & No         & All 23 OCPP commands \\
11 & Performance           & No         & Reconnect time (3$\times$ Hard Reset) \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Run all sections without EV (quick validation)}
...
\subsection{Run all sections with real EV}

\begin{lstlisting}[language=bash]
# Restore EV wait timeout:
